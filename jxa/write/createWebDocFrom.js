...
// Create a web document (picture, PDF, or web archive) from a web URL
// Usage: osascript -l JavaScript createWebDocFrom.js '<json>'
...
//
// Examples:
//   osascript -l JavaScript createWebDocFrom.js '{"url":"https://example.com/page"}'
//   osascript -l JavaScript createWebDocFrom.js '{"url":"https://example.com","name":"Page","database":"Inbox","readability":true}'
...
if (!jsonArg) {
  JSON.stringify({
    success: false,
    error: 'Usage: createWebDocFrom.js \'{"url":"https://..."}\''
  });
} else {
  try {
    ...
    const record = app.createWebDocumentFrom(url, options);

    if (!record) {
      throw new Error("Failed to create web document from URL");
    }
